          ...
          <div class="table-responsive text-nowrap">
            <table class="table table-hover">
              ...
              @php
                  ...
                  $year = DB::table('employee_works')->where('employee_id',$value->id)->orderBy('id','desc')->value('year');

                  $absence = (int)DB::table('employee_works')->where('employee_id',$value->id)->where('year',$year)->sum('absence'); // หยุด
                  $late = (int)DB::table('employee_works')->where('employee_id',$value->id)->where('year',$year)->sum('late'); // สาย
                  $dayoff = DB::table('dayoffs')->where('employee_id',$value->id)->where('year',$year)->value('dayoff'); // วันหยุดประจำปี
                  ...
                  $salary = DB::table('salarys')->where('employee_id',$value->id)->where('year',$year)->value('salary');
              ...
              <tbody class="table-border-bottom-0">
                <tr>
                  ...
                  <td>
                    ...
                    @else 
                      <a href="{{url('/admin/dayoff-by-branch')}}/{{$value->branch_id}}">กรอกวันหยุดประจำปี</a>
                    @endif
                  </td>
                  <td>
                    @if($bonus != 0)
                  ...
                  <td>
                    <div class="dropdown">
                      ...
                      <div class="dropdown-menu">
                        <a class="dropdown-item" href="{{url('/admin/edit-data-employee')}}/{{$value->branch_id}}/{{$value->id}}"><i class="bx bx-edit-alt me-1"></i> แก้ไขข้อมูล</a>
                        <a class="dropdown-item" href="{{url('/admin/information-employee')}}/{{$value->branch_id}}/{{$value->id}}"><i class="bx bx-folder-open"></i> ข้อมูลเพิ่มเติม</a>
                      </div>
                    </div>
                  </td>
                </tr>
